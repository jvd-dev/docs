...
### Request Simulação
Para a criação da proposta no sistema, utilizaremos a API de proposta no endpoint `/Proposta/fgts`, sendo necessário informar no corpo (body) o Id da simulação retornado no request de simulação. Veja o exemplo abaixo:


API Proposta (Homologação): https://api-propostas-teste-proj.zemafinanceira.com/
